...
// Empacotado, o binário vem de dentro do app.asar, onde não é executável — o
// electron-builder o extrai para app.asar.unpacked (ver asarUnpack no
// electron-builder.yml), então o path real precisa apontar pra lá.
const SEVEN_ZIP_BIN = path7z.replace("app.asar", "app.asar.unpacked");
...
export function allPartsReady(firstPartPath: string): boolean {
  ...
  if (/\.part0*1\.rar$/i.test(base)) {
    ...
    const nums = parts.map((p) =>
      parseInt(p.match(/\.part(\d+)\.rar$/i)?.[1] ?? "0", 10),
    );
...
async function runExtract(
  ...
  return new Promise((resolve, reject) => {
    ...
    // Binário 7-Zip completo bundlado com o app (7zip-bin-full) — não depende de
    // um `7z`/`unrar` instalado no sistema, e lê RAR nativamente (necessário para
    // os grupos .partNNN.rar comuns em hosts de arquivo).
    const proc = spawn(SEVEN_ZIP_BIN, args, { stdio: "pipe" });
    let stderr = "";
...
export function shouldAutoExtractFile(filePath: string): boolean {
  return (
    isExtractable(filePath) && (!isMultipart(filePath) || isFirstPart(filePath))
  );
}

function isMultipart(filePath: string): boolean {
  return (
    MULTIPART_RAR.test(filePath) ||
    PART_001.test(filePath) ||
    MULTIPART_ZIP.test(filePath)
  );
}
